...
standard_CV2 <- R6Class("standard_CV2", 
                       ...
                       public = list(
                         ...
                         run_decoding = function(place_holder){
                           ...
                           half_cores <- makeCluster(detectCores()/2)        
                           registerDoParallel(half_cores)
                           ...
                           #make all the codes below a sub function in run_decoding since they can share all the variable they have
                           single_run <-function(){
                             
                             #Let each run has their own results.
                             SINGLE_RESULTS <- NULL
                           ...
                           DECODING_RESULTS <- foreach(i = 1:16, .combine = 'rbind') %dopar% single_run()
                           # Try parLappy
                           #parLapply(cl,x,func)
